...
class BrematicProConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    @staticmethod
    async def _common_flow_handler(hass, user_input):
        """Handle common logic for user and options flows."""
        errors = {}

        if user_input is not None:
            entry = hass.config_entries.async_get_entry(hass.context["entry_id"]) if hass.context.get("entry_id") else None
        ...
        return hass.config_entries.async_show_form(
            step_id="user" if hass.context.get("entry_id") else "init",
